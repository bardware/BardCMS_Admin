...
REQUESTMETHOD: GET
FILENAME: order_thema_beitraege.php
FILETYPE: INCLUDE
*/

if(isset($arrBeitragRang)) { ?>
<form action="<?=$GETString;?>" method="post">
<? foreach($arrBeitragRang as $aBRKey => $aBRVal) { ?>
<input type="text" value="<?=$aBRVal["rang"];?>" size="5" name="rang[<?=$aBRKey;?>]"/> <?=$aBRVal["head"];?><br />
<? } ?>
<input type="submit" />
</form>
<? } ?>
